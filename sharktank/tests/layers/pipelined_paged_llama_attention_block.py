...
class ShardedPagedLlamaAttentionBlockTest(unittest.TestCase):
    ...
    def runTestSmallSizedLayer(self, dtype: torch.dtype):
        ...
        pipelined_cache = make_paged_kv_cache(shard_count=self.shard_count)
        ...
        def make_unsharded_and_sharded_equal_cache_states() -> (
            ...
            sharded_cache_state = pipelined_cache.shard_state(deepcopy(cache_state))
        ...
        actual_cache_state = unbox_tensor(
            ops.unshard(
                pipelined_cache.unflatten_page_tables(sharded_cache_state)
            ).flatten(start_dim=1)
        )
